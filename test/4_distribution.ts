...
describe('migrate() - Test out the entire logic with traded NFTs.', async () => {
	let lavaMigration: LavaMigration
	let lavaNft: LavaNft
	let lavaDistribution: LavaDistribution
	let USDC: Contract
	...
	describe('Lava NFT contract - Trasfer', async () => {
		it('Check NFT holders count', async () => {
			const maxtNftCount1 = await lavaNft.balanceOf(lavaMember_1.address)
			for (let i = 1; i < parseInt(maxtNftCount1 + ''); i++) {
				await lavaNft
					.connect(lavaMember_1)
					.transferFrom(lavaMember_1.address, lavaMember_2.address, i)
			}
			// console.log(
			// 	'1 - NFT balance:',
			// 	await lavaNft.balanceOf(lavaMember_1.address),
			// )
			// console.log(
			// 	'2 - NFT balance:',
			// 	await lavaNft.balanceOf(lavaMember_2.address),
			// )
			// console.log('NFT HOLDERS: ', await lavaNft.getNftHoldres())
	...
	describe('First distribution check - All claimed - No traded NFT.', async () => {
		...
		it('Perform distribution.', async () => {
			...
			expect(parseInt(calimablePayout1 / 1e12)).to.be.equal(
				finalUsdcOnMember1 - starterUsdcOnMember1,
			)
			expect(parseInt(calimablePayout2 / 1e12)).to.be.equal(
				finalUsdcOnMember2 - starterUsdcOnMember2,
			)
			expect(parseInt(calimablePayout3 / 1e12)).to.be.equal(
				finalUsdcOnMember3 - starterUsdcOnMember3,
			)
			expect(parseInt(remainingUsdcOnContract / 1e6)).to.be.equal(0)
		})
	})
})
